...
(function () {
  ...
  // ------------------------------------------------------------ appliance board
  // A picture of the kitchen rather than a list: each appliance is drawn, and lights up
  // when it is actually running. Everything is one colour (currentColor) plus a few named
  // parts the CSS lights separately, so state lives in CSS and this stays declarative.
  const ICONS = {
  ...
  function applianceIcon(family) {
    ...
    svg.innerHTML = ICONS[family] || ICONS.default;
    return svg;
  }
  ...
  function renderAppliances(rows) {
    ...
    for (const a of rows) {
      ...
      let when = "";
      if (!busy && a.status === "needed") when = "not scheduled yet";
