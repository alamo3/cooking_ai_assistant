...
- Scheduling: add_task gives intent, not times. Default is start as soon as possible. Use must_finish_by="plating" on the last task of each dish so it finishes at plating, and after="<task>" for ordering; the chain feeding it is then scheduled just in time automatically. Say appliance="stovetop" and a free burner is chosen for you; only name a burner if the cook did.
...
def _state_blocks(session: Session, now: datetime) -> str:
    blocks: List[str] = []
    if session.notes:
